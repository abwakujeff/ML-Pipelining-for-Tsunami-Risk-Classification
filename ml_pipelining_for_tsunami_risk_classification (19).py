...
# 6. Feature importance analysis (using Random Forest as example)
...
"""###Key Observations:
1. **Outstanding Performance:** Top models achieving 92-94% accuracy

2. **Best Models**: AdaBoost, Random Forest, and Gradient Boosting leading

3. **Feature Importance Surprise:** Year is the most important feature (27%) - this might indicate temporal patterns or data leakage

4. **Domain Knowledge Gap**: Magnitude only has 3% importance, which contradicts tsunami science
"""

# 7. Let's investigate the Year feature importance
print("Investigating Year feature patterns:")
print("=" * 40)

# Check tsunami frequency by year
yearly_analysis = RECORDS.groupby('Year').agg({
    'tsunami': ['mean', 'count'],
    'magnitude': 'mean'
}).round(3)

print("Yearly Tsunami Frequency and Magnitude:")
print(yearly_analysis.tail(10))  # Last 10 years

# 8. Create a more robust model without potential temporal leakage
# Let's build a model focusing on seismic characteristics only
seismic_features = ['magnitude', 'depth', 'latitude', 'longitude', 'sig', 'mmi', 'cdi']

X_seismic = RECORDS[seismic_features]
y_seismic = RECORDS['tsunami']

# Split the data
X_train_seis, X_test_seis, y_train_seis, y_test_seis = train_test_split(
    X_seismic, y_seismic, test_size=0.2, random_state=42, stratify=y_seismic
)

print("Seismic-only features shape:", X_train_seis.shape)

# 9. Train optimized models with seismic features only
...
gb_grid = GridSearchCV(gb_model, param_grid, cv=5, scoring='accuracy', n_jobs=-1)
gb_grid.fit(X_train_seis, y_train_seis)
